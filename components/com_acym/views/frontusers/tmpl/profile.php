...
?><div id="acym_fulldiv_acymodifyform" class="acym_front_page">
	...
	<form action="<?php echo acym_frontendLink('frontusers', true, acym_isNoTemplate()); ?>" method="post" name="adminForm" id="acymodifyform">
		<fieldset class="adminform acy_user_info">
            ...
            if (!empty($data['fields']) || (!empty($data['user']->id) && (!empty($exportButton) || !empty($deleteButton)))) {
                ?>
				<div id="acyuserinfo">
                    <?php
                    ...
                    foreach ($data['fields'] as $field) {
                        ...
                        echo $data['fieldClass']->displayField($field, $field->default_value, $size, $valuesArray, true, true, $data['user'], $field->frontend_profile);
                        echo '</span>';
                    }
						...
						<tr>
                            ...
                            if ($deleteButton == 1) {
								...
								<td id="acybutton_subscriber_delete_data">
									<button class="btn" onclick="if(confirm('<?php echo acym_translation('ACYM_ARE_YOU_SURE', true)."\\n".acym_translation('ACYM_DELETE_MY_DATA_CONFIRM', true); ?>')){ this.form.task.value='delete'; this.form.submit(); } return false;">
		...
		<input type="hidden" name="key" value="<?php echo $data['user']->key; ?>"/>
		<input type="hidden" name="ajax" value="1"/>

		<p class="acymodifybutton">
			<input class="btn btn-primary" type="submit" onclick="this.form.task.value='savechanges';return acym_checkChangeForm(this.form);" value="<?php echo $this->escape(acym_translation(empty($data['user']->id) ? 'ACYM_SUBSCRIBE' : 'ACYM_SAVE_CHANGES')); ?>"/>
		</p>
	</form>
