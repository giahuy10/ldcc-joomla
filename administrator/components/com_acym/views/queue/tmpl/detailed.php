    ...
    <form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm" data-abide novalidate>
        ...
        <?php } else { ?>
            ...
            <?php if (empty($data['allElements'])) { ?>
                <h1 class="cell acym__listing__empty__search__title text-center"><?php echo acym_translation_sprintf('ACYM_NO_X_FOUND', strtolower(acym_translation('ACYM_DETAILED_STAT'))); ?></h1>
            <?php } else { ?>
                <div class="grid-x acym__listing acym__listing__view__dqueue">
                    ...
                    <?php foreach ($data["allElements"] as $row) { ?>
                        <div elementid="<?php echo $row->id.'_'.$row->user_id; ?>" class="cell grid-x acym__listing__row">
                            ...
                            <div class="cell medium-1 text-center">
                                <i class="fa fa-trash-o acym_toggle_delete acym_delete_queue" table="queue" method="deleteOne" elementid="<?php echo $row->id.'_'.$row->user_id; ?>" confirmation="1"></i>
                            </div>
                        </div>
                    <?php } ?>
                </div>
            ...
            <?php } ?>
        <?php } ?>
        <?php echo acym_formOptions(false, 'detailed'); ?>
    </form>
</div>
