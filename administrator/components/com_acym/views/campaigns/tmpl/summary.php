...
<form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm" <?php echo $isSent ? '' : 'class="acym__form__campaign__edit"'; ?>>
        ...
        <div class="cell acym__content xxlarge-6 large-9">
            ...
            <div id="acym__campaign__summary" class="grid-x grid-margin-y">
                <div class="cell grid-x acym__campaign__summary__section margin-right-2">
                    ...
                    <div style="display: none" class="acym__hidden__mail__content"><?php echo $data['mailInformation']->body; ?></div>
                ...
                <div class="cell grid-x acym__campaign__summary__section">
                    ...
                    <p class="cell margin-top-1">
                        <?php
                        $textNumberOfRecipients = $isSent ? acym_translation('ACYM_CAMPAIGN_HAS_BEEN_SENT_TO_A_TOTAL_OF') : acym_translation('ACYM_CAMPAIGN_WILL_BE_SENT_TO_A_TOTAL_OF');

                        echo $textNumberOfRecipients.' '.acym_tooltip('<b>'.$data['nbSubscribers'].' '.strtolower(acym_translation('ACYM_RECIPIENTS')).'</b>', acym_translation('ACYM_SUMMARY_NUMBER_RECEIVERS_EXPLICATION')) ?>
                    </p>
                </div>
    ...
    <input type="hidden" value="<?php echo htmlspecialchars($data['campaignInformation']->id) ?>" name="id"/>
    <input type="hidden" value="<?php echo acym_date(htmlspecialchars($data['campaignInformation']->sending_date), 'j F Y H:s') ?>" name="sending_date"/>
