	...
	<div class="grid-x grid-margin-x">
		...
		<div class="cell medium-9 margin-top-1">
            ...
            echo acym_select(
                $ordering,
                'config[sendorder]',
                $data['config']->get('sendorder', 'subid, ASC'),
                'class="intext_select" onchange="if(this.value == \'rand\'){alert(\''.acym_translation('ACYM_NO_RAND_FOR_MULTQUEUE', true).'\')}"',
                'value',
                'text'
...
if (acym_level(1)) {
	...
	<div class="acym__configuration__cron acym__content acym_area padding-vertical-1 padding-horizontal-2 margin-bottom-2">
		...
		<div class="grid-x grid-margin-x">
			...
			<div class="cell medium-9">
				<a class="acym__color__blue" href="<?php echo $this->escape($cron_url); ?>" target="_blank"><?php echo $cron_url; ?></a>
			</div>
		</div>
	</div>
