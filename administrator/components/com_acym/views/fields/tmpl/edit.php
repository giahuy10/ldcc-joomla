...
?><form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div id="acym__fields__edit" class="acym__content grid-x cell">
		<div class="cell grid-x">
			...
			<button data-task="apply" class="button button-secondary medium-shrink small-12 margin-right-1 acy_button_submit"><?php echo acym_translation('ACYM_SAVE') ?></button>
			<button data-task="save" class="button acy_button_submit"><?php echo acym_translation('ACYM_SAVE_EXIT') ?></button>
		</div>
		<div class="cell grid-x grid-margin-x">
			<div class="medium-6 cell grid-x acym__fields__edit__field acym__content">
				...
				<label class="cell large-5"><?php echo acym_translation('ACYM_FIELD_TYPE') ?>
                    <?php
                    echo acym_select($data['fieldType'], 'field[type]', $data['field']->type, 'class="acym__fields__edit__select"', 'value', 'name');
                    ?>
				</label>
                ...
                <?php if (empty($data['field']->id) || $data['field']->id != 2) { ?>
					<div class="cell grid-x large-6  margin-top-1 acym__fields__change" id="acym__fields__required">
                        <?php echo acym_switch('field[required]', $data['field']->required, acym_translation('ACYM_REQUIRED'), array(), 'shrink', 'auto', 'tiny margin-0'); ?>
					</div>
                <?php } ?>
				<div class="cell grid-x xlarge-6 margin-top-2 acym__fields__change" id="acym__fields__editable-user-creation">
				...
				<div class="cell grid-x xlarge-6 margin-top-2 acym__fields__change" id="acym__fields__editable-user-modification">
				...
				<label class="cell margin-top-2 large-11 acym__fields__change" id="acym__fields__error-message"><?php echo acym_translation('ACYM_CUSTOM_ERROR') ?>
					<input type="text" name="field[option][error_message]" value="<?php echo empty($data['field']->option->error_message) ? '' : $data['field']->option->error_message; ?>">
				</label>
				<label class="cell margin-top-1 acym__fields__change" id="acym__fields__authorized-content"><?php echo acym_translation('ACYM_AUTHORIZED_CONTENT') ?>
                    <?php
                    $authorizedContent = array('all' => acym_translation('ACYM_ALL'), 'number' => acym_translation('ACYM_NUMBER_ONLY'), 'letters' => acym_translation('ACYM_LETTERS_ONLY'), 'numbers_letters' => acym_translation('ACYM_NUMBERS_LETTERS_ONLY'), 'regex' => acym_translation('ACYM_REGULAR_EXPRESSION').' <input type="text" name="field[option][authorized_content][regex]">',);
				...
				<label class="cell margin-top-1 large-5 acym__fields__change" id="acym__fields__format"><?php
                    ...
                    ?>
					<input type="text" name="field[option][format]" value="<?php echo empty($data['field']->option->format) ? '' : $data['field']->option->format; ?>">
				</label>
				...
				<div class="cell grid-x acym__fields__change large-11 acym__content margin-bottom-2" id="acym__fields__value">
					<div class="grid-x acym__listing acym__fields__values__listing__sortable">
                        ...
                        <?php if (empty($data['field']->value)) { ?>
							<div class="grid-x cell acym__fields__value__sortable acym__content margin-bottom-1">
								<input type="text" name="field[value][value][]" class="cell medium-4 margin-right-1">
								<input type="text" name="field[value][title][]" class="cell medium-4 margin-right-1">
								<div class="cell medium-3 grid-x">
									...
									<div class="cell small-6 text-right">
										<i class="material-icons cursor-pointer acym__sortable__field__edit__handle">drag_handle</i>
									</div>
								</div>
							</div>
                        <?php } else {
                            ...
                            foreach ($data['field']->value as $value) { ?>
								<div class="grid-x cell acym__fields__value__sortable acym__content margin-bottom-1">
									<input type="text" name="field[value][value][]" class="cell medium-4 margin-right-1" value="<?php echo $value->value ?>">
									<input type="text" name="field[value][title][]" class="cell medium-4 margin-right-1" value="<?php echo $value->title ?>">
									<div class="cell medium-3 grid-x">
										<div class="cell small-6">
				...
				<div class="cell large-11 grid-x acym__fields__change acym__content" id="acym__fields__from-db">
					...
					<label class="cell margin-top-1 medium-3 margin-right-1"><?php echo acym_translation('ACYM_WHERE_OPERATION') ?>
                        <?php echo acym_select($data['typeWhere'], 'fieldDB[where_sign]', empty($data['field']->fieldDB->where_sign) ? '' : $data['field']->fieldDB->where_sign, 'class="acym__fields__edit__select"', 'value', 'name') ?>
					</label>
			...
			<div class="medium-6 cell grid-x acym__fields__edit__back-front">
				...
				<div class="cell acym__content margin-top-2  grid-x">
