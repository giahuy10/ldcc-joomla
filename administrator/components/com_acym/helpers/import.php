...
class acymimportHelper
{
    ...
    function file()
    {
        ...
        if (!preg_match('#^(csv)$#Ui', $extension) || preg_match('#\.(php.?|.?htm.?|pl|py|jsp|asp|sh|cgi)$#Ui', $importFile['name'])) {
            acym_enqueueNotification(acym_translation_sprintf('ACCEPTED_TYPE', htmlspecialchars($extension, ENT_COMPAT, 'UTF-8'), $config->get('allowed_files')), 'error');
        ...
        if ($fileError > 0) {
            switch ($fileError) {
                case 1:
                    ...
                    return false;
                case 2:
                    acym_enqueueNotification(acym_translation('ACYM_UPLOADED_FILE_EXCEED_MAX_FILESIZE_HTML_FORM'), 'error');
        ...
        if (!acym_uploadFile($importFile['tmp_name'], $uploadPath.$attachment->filename)) {
            if (!move_uploaded_file($importFile['tmp_name'], $uploadPath.$attachment->filename)) {
                acym_enqueueNotification(acym_translation_sprintf('ACYM_FAIL_UPLOAD', '<b><i>'.htmlspecialchars($importFile['tmp_name'], ENT_COMPAT, 'UTF-8').'</i></b>', '<b><i>'.htmlspecialchars($uploadPath.$attachment->filename, ENT_COMPAT, 'UTF-8').'</i></b>'), 'error');
            }
        }
    ...
    function cms()
    {
        ...
        acym_query('UPDATE #__acym_configuration SET `value` = '.$time.' WHERE `name` = \'last_import\'');
    ...
    function database()
    {
        ...
        $fields = acym_getColumnsNonAcyTable($table);
        if (empty($fields)) {
            acym_enqueueNotification(acym_translation('ACYM_SPECIFYTABLE'), 'warning', 5000);
        ...
        if (empty($select['`creation_date`'])) {
            $select['`creation_date`'] = "'".acym_date('now', 'Y-m-d H:i:s')."'";
        }
        ...
        $select['`source`'] = "'import_".$time."'";

        $query = 'INSERT IGNORE INTO #__acym_user ('.implode(' , ', array_keys($select)).') SELECT '.implode(' , ', $select).' FROM '.$table.' WHERE '.$select['`email`'].' LIKE \'%@%\'';
    ...
    public function _handleContent(&$contentFile)
    {
        ...
        if (!$this->_autoDetectHeader()) {
            acym_enqueueNotification(acym_translation_sprintf('ACYM_IMPORT_HEADER', htmlspecialchars($this->header, ENT_COMPAT, 'UTF-8')), 'error');
    ...
    function _insertUsers($users, $timestamp)
    {
        ...
        $query = 'INSERT'.($this->overwrite ? '' : ' IGNORE').' INTO #__acym_user (`'.implode('`,`', $colNames).'`) VALUES (';
        $values = array();
        $allemails = array();
        foreach ($users as $a => $oneUser) {
            $value = array();
            foreach ($oneUser as $map => $oneValue) {
                if ($map == 'active' && !empty($this->importblocked) && $this->importblocked == true) {
                    ...
                    $value[] = acym_escapeDB($oneValue);
                } else {
                    $value[] = $oneValue;
                }
                if ($map == 'email') {
                    $allemails[] = acym_escapeDB($oneValue);
                }
            }
        ...
        acym_query('UPDATE #__acym_configuration SET `value` = '.$timestamp.' WHERE `name` = \'last_import\'');

        $this->allSubid = array_merge($this->allSubid, acym_loadResultArray('SELECT id FROM #__acym_user WHERE email IN ('.implode(',', $allemails).')'));
    ...
    function _autoDetectHeader()
    {
        ...
        foreach ($this->columns as $i => $oneColumn) {
            ...
            if (in_array($this->columns[$i], array('listids', 'listname'))) {
                continue;
            }
            if (!in_array($this->columns[$i], $columns) && $this->columns[$i] != 1) {
                acym_enqueueNotification(acym_translation_sprintf('ACYM_IMPORT_ERROR_FIELD', '<b>'.htmlspecialchars($this->columns[$i], ENT_COMPAT, 'UTF-8').'</b>', '<b>'.implode('</b> | <b>', array_diff($columns, array('id', 'cms_id'))).'</b>'), 'error');
    ...
    function _subscribeUsers()
    {
        ...
        if (!empty($this->importUserInLists)) {
            foreach ($this->importUserInLists as $listid => $arrayEmails) {
                ...
                $query .= "SELECT $listid,`id`,'$subdate',1 FROM #__acym_user WHERE `email` IN (";
                ...
                foreach ($this->allSubid as $subid) {
                    ...
                    if ($b > 200) {
                        ...
                        if ($val == -1) {
                            ...
                            $nbsubscribed = -acym_loadResult('SELECT COUNT(*) FROM #__acym_listsub WHERE `list_id` = '.$listid.' AND status != -1 AND `user_id` IN ('.implode(',', $currentSubids).')');
                        }
                        $affected = acym_query($query);
