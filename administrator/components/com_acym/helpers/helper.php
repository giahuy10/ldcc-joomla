...
define('ACYM_SPAMURL', ACYM_ACYWEBSITE.'index.php?option=com_updateme&ctrl=spamsystem&task=');
define('ACYM_HELPURL', ACYM_ACYWEBSITE.'index.php?option=com_updateme&ctrl=doc&component='.ACYM_NAME.'&page=');
define('ACYM_REDIRECT', ACYM_ACYWEBSITE.'index.php?option=com_updateme&ctrl=redirect&page=');
...
function acym_initializeChart()
{
    ...
    if (!$loaded) {
        acym_addScript(false, ACYM_JS.'libraries/chart.min.js?v='.filemtime(ACYM_MEDIA.'js'.DS.'libraries'.DS.'chart.min.js'));
        $loaded = true;
    }
}
...
function acym_radio($options, $name, $selected = null, $id = null, $attributes = array(), $objValue = 'value', $objText = 'text', $useIncrement = false)
{
    ...
    foreach ($options as $value => $label) {
        ...
        foreach ($attributes as $attribute => $val) {
            $return .= ' '.$attribute.'="'.htmlspecialchars($val, ENT_QUOTES, 'utf-8').'"';
        }
...
function acym_modal($button, $data, $id = null, $attributesModal = '', $attributesButton = '', $isButton = true)
{
    if (empty($id)) {
    ...
    $modal .= '<div class="reveal" id="'.$id.'" '.$attributesModal.' data-reveal>';
    $modal .= $data;
...
function acym_modal_include($button, $file, $id = null, $data, $attributes = '')
{
    if (empty($id)) {
        $id = 'acymodal_'.rand(1000, 9000);
    }

    $modal = '<div data-open="'.$id.'">'.$button;
    $modal .= '<div class="reveal" id="'.$id.'" '.$attributes.' data-reveal>';
    ob_start();
    include($file);
    $modal .= ob_get_clean();
    $modal .= '<button type="button" class="close-button" data-close aria-label="Close reveal" type="button">';
    $modal .= '<span aria-hidden = "true" >&times;</span >';
...
function acym_modal_pagination_lists_import($button, $class, $textButtonRight, $id = null, $attributes = '')
{
    if (empty($id)) {
    ...
    $data = '
            ...
            <div class="cell grid-x">
              ...
              <div class="cell grid-margin-x large-up-2 small-up-1" id="modal__pagination__listing__button-submit">   
                <button class="cell button acym__users__import__generic__import__button" id="lists">'.$textButtonRight.'</button>
              </div>';

    $modal = '<button type="button" data-open="'.$id.'" class="modal__pagination__button-open button '.$class.'" '.$attributes.'>'.$button.'</button>';
...
function acym_displayDateFormat($format, $name = 'date', $default = '14/06/1997')
{
    ...
    foreach ($formatToDisplay as $one) {
        if ($one == 'd') {
            $return .= '<div class="medium-3 cell">'.acym_select($days, $name, $defaultDate[$i], 'class="acym__custom__fields__select__form"', 'value', 'text').'</div>';
        }
        if ($one == 'm') {
            $return .= '<div class="medium-5 cell">'.acym_select($month, $name, $defaultDate[$i], 'class="acym__custom__fields__select__form "', 'value', 'text').'</div>';
        }
        if ($one == 'y') {
            $return .= '<div class="medium-4 cell">'.acym_select($year, $name, $defaultDate[$i], 'class="acym__custom__fields__select__form "', 'value', 'text').'</div>';
        }
        $i++;
    }
...
function acym_selectMultiple($data, $name, $selected = array(), $attribs = array(), $optValue = "value", $optText = "text", $translate = false)
{
    ...
    foreach ($data as $oneDataKey => $oneDataValue) {
        if (is_object($oneDataValue)) {
            ...
            if (isset($oneDataValue->disable)) {
                $disabled = $oneDataValue->disable;
            }
        } else {
            $value = $oneDataKey;
            $text = $oneDataValue;
        ...
        if (strtolower($value) == '<optgroup>') {
            ...
            $dropdown .= '<option value="'.$value.'"'.(in_array($value, $selected) ? ' selected="selected"' : '').(isset($disabled) ? ' disabled="disabled"' : '').">".$text.'</option>';
        }
    }
...
function acym_getColumns($table, $acyTable = true)
{
    $prefix = $acyTable ? '#__acym_' : '#__';

    return acym_loadResultArray('SHOW COLUMNS FROM '.$prefix.$table);
}

function acym_getColumnsNonAcyTable($table)
{
...
function acym_initModule($params = null)
{
    ...
    echo "\n".'<link rel="stylesheet" type="text/css" href="'.ACYM_CSS.'module.min.css?v='.str_replace('.', '', $config->get('version')).'" ></script>'."\n";
}
...
function acym_importFile($file, $uploadPath, $onlyPict, $maxwidth = '')
{
    ...
    if ($file["error"] > 0) {
        ...
        acym_enqueueNotification("Error Uploading file: ".$phpFileUploadErrors[$file["error"]], 'error', 5000);
...
function acym_getFilesFolder($folder = 'upload', $multipleFolders = false)
{
    ...
    $config = acym_config();
    if ($folder == 'upload') {
        $uploadFolder = $config->get('uploadfolder', ACYM_MEDIA_FOLDER.'/upload');
    } else {
        $uploadFolder = $config->get('mediafolder', ACYM_MEDIA_FOLDER.'/upload');
    }
...
{
    if (!empty($link)) {
        $text = '<a href="'.$link.'" title="'.htmlspecialchars($title, ENT_QUOTES, 'UTF-8').'"">'.$text.'</a>';
    }
...
function acym_deleteFolder($path)
{
    ...
    if (!is_dir($path)) {
        acym_enqueueNotification($path.' is not a folder', 'error', 0);
    ...
    if (@rmdir($path)) {
        ...
        acym_enqueueNotification('Could not delete folder '.$path, 'error', 0);
        $ret = false;
    }
...
function acym_copyFolder($src, $dest, $path = '', $force = false, $use_streams = false)
{
    ...
    if (!file_exists($src)) {
        acym_enqueueNotification('Folder '.$src.' does not exist', 'error', 0);

        return false;
    }

    if (file_exists($dest) && !$force) {
        acym_enqueueNotification('Folder '.$dest.' already exists', 'error', 0);

        return true;
    }

    if (!acym_createFolder($dest)) {
        acym_enqueueNotification('Cannot create destination folder', 'error', 0);

        return false;
    }

    if (!($dh = @opendir($src))) {
        acym_enqueueNotification('Cannot open source folder', 'error', 0);
    ...
    while (($file = readdir($dh)) !== false) {
        ...
        switch (filetype($sfid)) {
            ...
            case 'file':
                if (!@copy($sfid, $dfid)) {
                    acym_enqueueNotification('Copy file '.$sfid.' failed, check permissions', 'error', 0);
...
function acym_moveFolder($src, $dest, $path = '', $use_streams = false)
{
    ...
    if (!@rename($src, $dest)) {
        acym_enqueueNotification('Could not move folder '.$src.' to '.$dest.', check permissions', 'error', 0);
...
function acym_deleteFile($file)
{
    ...
    if (!is_file($file)) {
        acym_enqueueNotification($file.' is not a file', 'error', 0);
    ...
    if (!@unlink($file)) {
        ...
        acym_enqueueNotification('Failed to delete '.$filename, 'error', 0);
...
function acym_moveFile($src, $dest, $path = '', $use_streams = false)
{
    ...
    if (!is_readable($src)) {
        acym_enqueueNotification('Could not find source file, check permissions: '.$src, 'error', 0);

        return false;
    }

    if (!@rename($src, $dest)) {
        acym_enqueueNotification('Could not move the file', 'error', 0);
...
function acym_uploadFile($src, $dest)
{
    ...
    if (is_writeable($baseDir) && move_uploaded_file($src, $dest)) {
        if (@chmod($dest, octdec('0644'))) {
            ...
            acym_enqueueNotification('The file has been rejected for safety reason', 'error', 0);
        }
    } else {
        acym_enqueueNotification('Couldn\'t upload file, check permissions for the folder '.$baseDir, 'error', 0);
    }
...
function acym_copyFile($src, $dest, $path = null, $use_streams = false)
{
    ...
    if (!@copy($src, $dest)) {
        acym_enqueueNotification('Could not copy the file '.$src.' to '.$dest, 'error', 0);
...
function acym_popup($url, $text, $class = '', $width = 800, $height = 500, $id = '', $params = '')
{
    static $loaded = false;
    ...
    if (!$loaded) {
        acym_addScript(
            false,
            ACYM_JS.'acym.js?v='.filemtime(ACYM_MEDIA.'js'.DS.'acym.js')
        );
        acym_addStyle(
            false,
            ACYM_CSS.'acypopup.css?v='.filemtime(ACYM_MEDIA.'css'.DS.'acypopup.css')
        );
        acym_addStyle(false, ACYM_CSS.'acyicon.css?v='.filemtime(ACYM_MEDIA.'css'.DS.'acyicon.css'));
        $loaded = true;
    }
    ...
    return '<a onclick="acym.openpopup(\''.$url.'\','.$width.','.$height.'); return false;" class="acympopup '.$class.'" '.$id.$params.'>'.$text.'</a>';
}
...
function acym_switchFilter($switchOptions, $selected, $name)
{
    echo '<input type="hidden" id="acym__type-template" name="'.$name.'" value="'.$selected.'">';
    ...
    foreach ($switchOptions as $value => $text) {
        ...
        echo '<button class="'.$class.'" type="'.$value.'">'.acym_translation($text).'</button>';
    }
}
...
function acym_filterFormat($name, $selected)
{
    echo '<input type="hidden" id="acym_listingformat" name="'.$name.'" value="'.$selected.'"/>';
    echo acym_tooltip('<i format="list" class="acym__format__view material-icons acym__format__view__list'.($selected == 'list' ? ' acym__format__view--current' : '').'">list</i>', acym_translation('ACYM_LIST_VIEW'));
    echo acym_tooltip('<i format="block" class="acym__format__view material-icons acym__format__view__block'.($selected != 'list' ? ' acym__format__view--current' : '').'">view_module</i>', acym_translation('ACYM_GRID_VIEW'));
}
...
function acym_switch($name, $value, $label = null, $attrInput = array(), $labelClass = 'medium-6 small-9', $switchContainerClass = "auto", $switchClass = "tiny", $toggle = null, $toggleOpen = true)
{
    ...
    foreach ($attrInput as $oneAttributeName => $oneAttributeValue) {
        $switch .= ' '.$oneAttributeName.'="'.htmlspecialchars($oneAttributeValue, ENT_QUOTES, 'UTF-8').'"';
    }
    $switch .= '>';
    $switch .= '
...
function acym_selectTemplates($mailOptions, $selected)
{
        ...
        $button = '<i class="fa fa-check-circle '.$iconSelectedClass.'"></i>
                ...
                <div class="cell grid-x text-center">
                    <div class="cell acym__templates__pic text-center">
                        <img src="'.htmlspecialchars($oneTpl->thumbnail).'" alt="'.htmlspecialchars($oneTpl->name).'" />
                    </div>
...
function acym_sortBy($options = array(), $listing, $default = "")
{
    ...
    $orderingSortOrder = acym_getVar('string', $listing.'_ordering_sort_order', 'asc');
...
function acym_enqueueNotification($message, $type = 'info', $time = 0)
{
    ...
    $notification .= '<i class="fa '.$logo.'"></i><p>'.$message.'</p><i class="fa fa-close"></i></div>';
...
function acym_trigger($method, $args = array(), $plugin = null)
{
    ...
    if (!empty($args[1]) && is_object($args[1]) && !empty($args[1]->id) && empty($args[1]->key)) {
        $args[1]->key = acym_generateKey(14);
        $userClass = acym_get('class.user');
        $userClass->sendConf = false;
        $userClass->save($args[1]);
    }
