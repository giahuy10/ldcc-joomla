...
class QueueController extends acymController
{
    ...
    {
        $active = acym_getVar("string", "acym__queue__play_pause__active__new_value");
        $campaignId = acym_getVar("string", "acym__queue__play_pause__campaign_id");

        if (!empty($campaignId)) {
            acym_query("UPDATE #__acym_campaign SET active = ".$active." WHERE id = ".$campaignId);
        } else {
            if ($active) {
