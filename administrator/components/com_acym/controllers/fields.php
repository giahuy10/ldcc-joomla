...
class FieldsController extends acymController
{
    ...
    public function edit()
    {
        ...
        if (!empty($id)) {
            $this->breadcrumb[htmlspecialchars($field->name)] = acym_completeLink('fields&task=edit&id='.$id);
        } else {
    ...
    public function setColumns()
    {
        ...
        echo json_encode(acym_loadResultArray($query));
        exit;
    }
    ...
    public function apply()
    {
        ...
        if (!empty($id)) {
            acym_setVar('id', $id);
            acym_enqueueNotification(acym_translation('ACYM_CUSTOM_FIELD_SAVE'), 'success', 5000);
        } else {
