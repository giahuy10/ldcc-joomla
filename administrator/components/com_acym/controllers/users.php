...
class UsersController extends acymController
{
    ...
    public function edit()
    {
        ...
        if (!empty($userId)) {
            ...
            if (!empty($userStatFromDB)) {
                ...
                $userStat->pourcentageOpen = intval(($userStat->open * 100) / $userStat->totalSent);
        ...
        foreach ($allFields as $one) {
            ...
            if ($one->id == 1) {
                $defaultValue = htmlspecialchars(empty($userData['user-information']->id) ? '' : $userData['user-information']->name, ENT_QUOTES, 'UTF-8');
            } else if ($one->id == 2) {
                $defaultValue = htmlspecialchars(empty($userData['user-information']->id) ? '' : $userData['user-information']->email, ENT_QUOTES, 'UTF-8');
    ...
    public function doexport()
    {
        ...
        $charset = acym_getVar('string', 'export_charset');
        ...
        $newConfig->export_fields = implode(',', $fieldsToExport);
        if (empty($selectedUsers)) {
        ...
        $exportHelper = acym_get('helper.export');
        $exportHelper->exportCSV($query, $fieldsToExport, $separator, $charset);
    ...
    {
        acym_enqueueNotification($message, 'error', 0);
    ...
    public function save()
    {
        ...
        if (empty($matches)) {
            $this->edit();
            acym_enqueueNotification(acym_translation_sprintf('ACYM_WRONG_EMAIL_ADDRESS', $user->email), 'error', 0);
    ...
    public function getColumnsFromTable()
    {
        ...
        $columns = acym_getColumnsNonAcyTable($tableName);
    ...
    public function setAjaxListing()
    {
        ...
        if (empty($users['users'])) {
            $return .= '<h1 class="cell acym__listing__empty__search__modal text-center">'.acym_translation_sprintf('ACYM_NO_X_FOUND', strtolower(acym_translation('ACYM_USER'))).'</h1>';
        }
