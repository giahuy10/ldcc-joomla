...
class acymqueueClass extends acymClass
{
    public function getMatchingCampaigns($settings)
    {
        ...
        $queryCount = 'SELECT COUNT(campaign.mail_id) '.$query;
    ...
    public function getMatchingResults($settings)
    {
        $query = 'FROM #__acym_queue AS queue 
                    ...
                    JOIN #__acym_campaign AS campaign ON mail.id = campaign.mail_id 
        ...
        $query = 'SELECT mail.id, queue.sending_date, mail.name, mail.subject, user.email, user.name AS user_name, queue.user_id, queue.try, campaign.active '.$query.' ORDER BY queue.sending_date ASC';
    ...
    public function scheduleReady()
    {
        ...
        acym_query('UPDATE #__acym_campaign SET sent = 1 WHERE mail_id IN ('.implode(',', array_keys($mailReady)).')');
    ...
    public function getReady($limit, $mailid = 0)
    {
        ...
        $query .= ' WHERE queue.`sending_date` <= '.acym_escapeDB(date('Y-m-d H:i:s')).' AND campaign.`active` = 1 AND user.active = 1';
