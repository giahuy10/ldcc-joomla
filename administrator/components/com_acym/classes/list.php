...
class acymlistClass extends acymClass
{
    ...
    public function getMatchingLists($settings)
    {
        ...
        if (!empty($settings['ordering']) && !empty($settings['ordering_sort_order'])) {
            $query .= ' ORDER BY list.'.acym_secureDBColumn($settings['ordering']).' '.strtoupper($settings['ordering_sort_order']);
        }
    ...
    {
        $query = 'SELECT user_id FROM #__acym_user_has_list WHERE list_id = '.$listId;
    ...
    public function save($list)
    {
        ...
        if (empty($list->id) && empty($list->cms_user_id)) {
    ...
    public function setVisible($elements, $status)
    {
        ...
        acym_query('UPDATE #__acym_list SET visible = '.$status.' WHERE id IN ('.implode(',', $elements).')');
    }
