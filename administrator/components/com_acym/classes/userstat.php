...
class acymuserStatClass extends acymClass
{
    ...
    public function save($userStat)
    {
        ...
        if (!empty($userStat['open_date'])) {
            $onDuplicate[] = 'open_date = "'.$userStat['open_date'].'"';
        }
    ...
    {
        $query = 'SELECT * FROM #__acym_user_stat WHERE `mail_id` = '.$mail_id.' AND `user_id`='.$user_id;
    ...
    {
        $query = 'SELECT * FROM #__acym_user_stat WHERE user_id = '.$idUser;
    ...
    public function getDetailedStats($settings)
    {
        ...
        if (empty(!$settings['mail_id'])) {
            $where[] = 'us.mail_id = '.$settings['mail_id'];
        }

        if (empty(!$settings['search'])) {
            $where[] = '(m.name LIKE "%'.$settings['search'].'%" OR u.email LIKE "%'.$settings['search'].'%")';
        }
