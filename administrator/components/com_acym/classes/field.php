...
class acymfieldClass extends acymClass
{
    ...
    {
        $query = 'SELECT * FROM #__acym_field ORDER BY '.acym_secureDBColumn('ordering').' '.strtoupper('ASC');
    ...
    {
        $query = 'SELECT * FROM #__acym_field WHERE `id` IN('.implode(',', $ids).') ORDER BY '.acym_secureDBColumn('ordering').' '.strtoupper('ASC');
    ...
    public function displayField($field, $defaultValue, $size, $valuesArray, $displayOutside = true, $displayFront = false, $user = null, $dontDisplay = 1, $displayIf = '')
    {
        if ($dontDisplay == 0) {
            return '';
        }
        ...
        if ($displayFront && !empty($user->id)) {
            ...
            if ($field->id == 1) {
                ...
                $allValues = array();
                $defaultValue = $this->getFieldsValueByUserId($user->id);
                if (!empty($defaultValue)) {
                    foreach ($defaultValue as $one) {
        ...
        $return = '';
        if ($field->id == 1) {
            $return .= ($displayOutside ? '<label class="cell"><h6 class="acym__users__creation__fields__title">'.acym_translation('ACYM_NAME').'</h6>' : '').'<input '.($displayOutside ? '' : 'placeholder="'.$field->name.'"').' type="text" class="cell" name="user[name]" placeholder="'.acym_translation('ACYM_NAME').'" value="'.$defaultValue.'">'.($displayOutside ? '</label>' : '');
        } else if ($field->id == 2) {
            $return .= ($displayOutside ? '<label class="cell"><h6 class="acym__users__creation__fields__title">'.acym_translation('ACYM_EMAIL').'</h6>' : '').'<input '.($displayOutside ? '' : 'placeholder="'.$field->name.'"').' id="acym__user__edit__email" required type="email" class="cell" name="user[email]" '.($displayFront && $cmsUser ? 'disabled' : '').' placeholder="'.acym_translation('ACYM_EMAIL').'" value="'.$defaultValue.'">'.($displayOutside ? '</label>' : '');
            ...
            $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>' : '').'<input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' '.($displayOutside ? '' : 'placeholder="'.$field->name.'"').' '.$authorizedContent.' style="'.$size.'" type="text" name="customField['.$field->id.']" value="'.$defaultValue.'">'.($displayOutside ? '</label>' : '');
        } else if ($field->type == 'textarea') {
            $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>' : '').'<textarea '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' rows="'.$field->option->rows.'" cols="'.$field->option->columns.'" name="customField['.$field->id.']">'.(empty($defaultValue) ? $field->name : $defaultValue).'</textarea>'.($displayOutside ? '</label>' : '');
        } else if ($field->type == 'radio') {
            if ($displayFront) {
                $return .= '<div '.$displayIf.' class="cell acym__content"><label class="cell"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>';
                ...
                foreach ($valuesArray as $key => $value) {
                    ...
                    $return .= '<label>'.$value.'<input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' type="radio" name="customField['.$field->id.']" value="'.$key.'" '.$defaultValue.'></label>';
                }
                $return .= '<div>';
            } else {
                $return .= '<div '.$displayIf.' class="cell acym__content"><label class="cell"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>'.acym_radio($valuesArray, 'customField['.$field->id.'][]', empty($defaultValue) ? null : (is_array($defaultValue) ? $defaultValue[0] : $defaultValue), null, ($field->required ? array('data-required' => json_encode(array('type' => $field->type, 'message' => $field->option->error_message))) : array())).'</label></div>';
            }
            ...
            if ($displayFront) {
                ...
                foreach ($valuesArray as $key => $value) {
                    $defaultValue = $defaultValue == $key ? 'checked' : '';
                    $return .= '<label>'.$value.'<input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' type="checkbox" name="customField['.$field->id.']" value="'.$key.'" '.$defaultValue.'></label>';
                }
            } else {
                ...
                foreach ($valuesArray as $key => $value) {
                    $return .= !empty($defaultValue->$key) ? '<label>'.$value.'<input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' type="checkbox" name="customField['.$field->id.']['.$value.']" class="acym__users__creation__fields__checkbox" checked></label>' : '<label class="cell margin-top-1">'.$value.'<input type="checkbox" name="customField['.$field->id.']['.$value.']" class="acym__users__creation__fields__checkbox"></label>';
                }
            }
            $return .= '</div>';
            ...
            $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>' : '').acym_select($valuesArray, 'customField['.$field->id.']', empty($defaultValue) ? '' : $defaultValue, 'class="acym__custom__fields__select__form" style="'.$size.'"'.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : ''), 'value', 'text').($displayOutside ? '</label>' : '');
        } else if ($field->type == 'multiple_dropdown') {
            $defaultValue = is_array($defaultValue) ? $defaultValue : array($defaultValue);
            $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1"><h6 class="acym__users__creation__fields__title">'.$field->name.'</h6>' : '').acym_selectMultiple($valuesArray, 'customField['.$field->id.']', empty($defaultValue) ? array() : $defaultValue, $field->required ? array('data-required' => json_encode(array('type' => $field->type, 'message' => $field->option->error_message)), 'class' => 'acym__custom__fields__select__multiple__form', 'style' => $size) : array('class' => 'acym__custom__fields__select__multiple__form', 'style' => $size)).($displayOutside ? '</label>' : '');
            ...
            if ($displayFront) {
                $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1 grid-x grid-margin-x"><h6 class="acym__users__creation__fields__title cell">'.$field->name.'</h6>' : '').'<input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' type="file" name="customField['.$field->id.']"></label>';
            } else {
                $return .= acym_inputFile('customField['.$field->id.'][]', $defaultValue, '', '', $field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '');
            }
        } else if ($field->type == 'phone') {
            $return .= ($displayOutside ? '<label '.$displayIf.' class="cell margin-top-1 grid-x grid-margin-x"><h6 class="acym__users__creation__fields__title cell">'.$field->name.'</h6>' : '').'<div class="medium-3">'.acym_generateCountryNumber('customField['.$field->id.'][code]', empty($defaultValue) ? '' : $defaultValue->code).'</div><input '.($field->required ? 'data-required=\''.json_encode(array('type' => $field->type, 'message' => $field->option->error_message)).'\'' : '').' '.($displayOutside ? '' : 'placeholder="'.$field->name.'"').' style="'.$size.'" class="medium-9 cell" type="tel" name="customField['.$field->id.'][phone]" value="'.(empty($defaultValue) ? '' : $defaultValue->phone).'" data-format="'.$field->option->format.'">'.($displayOutside ? '</label>' : '');
